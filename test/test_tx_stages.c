...
static void test_ht_scramble(int mcs) {
    ...
    for (int byte_idx = 0; byte_idx < PSDU_LEN; byte_idx++) {
        for (int bit = 0; bit < 8; bit++) {
            input[SERVICE_BITS + byte_idx * 8 + bit] = (TEST_PSDU[byte_idx] >> bit) & 1;
        }
    }
    ...
    if ((size_t)n_total != vec->n_bits) {
        TEST_FAIL("length mismatch: computed %d, vector has %zu", n_total, vec->n_bits);
        free(input);
        free(scrambled);
        vector_free(vec);
        return;
...
static void test_ht_encode(int mcs) {
    ...
    size_t n_punctured = lib80211_puncture(coded, punctured, n_coded, info->cr_n, info->cr_d);
    if (n_punctured == 0) {
        TEST_FAIL("puncture returned 0 (unsupported rate %d/%d?)", info->cr_n, info->cr_d);
        free(coded);
        free(punctured);
        vector_free(scr_vec);
        vector_free(enc_vec);
    ...
    size_t compare_len = n_punctured < enc_vec->n_bits ? n_punctured : enc_vec->n_bits;

    if (n_punctured != enc_vec->n_bits) {
        TEST_FAIL("encoded length mismatch: got %zu, expected %zu", n_punctured, enc_vec->n_bits);
