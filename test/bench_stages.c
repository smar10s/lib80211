...
static void bench_equalize(void *ctx) {
    eq_ctx *c = (eq_ctx *)ctx;
    lib80211_equalize(c->Y_re, c->Y_im, c->H_re, c->H_im, c->noise_var, c->out_re, c->out_im);
}
...
static void bench_viterbi(void *ctx) {
    ...
    lib80211_viterbi_decode(c->soft_bits, c->out_bits, c->n_coded_bits, c->n_data_bits);
}
...
static void bench_ldpc(void *ctx) {
    ...
    lib80211_ldpc_decode(c->llr_in, c->decoded_bits, c->cw_len, c->rate_n, c->rate_d, c->max_iters);
}
...
int main(void) {
    ...
    lib80211_estimate_channel(plan, ltf_start_re, ltf_start_im, H_re, H_im, &noise_var);
    if (noise_var < 1e-10f)
        noise_var = 0.001f; /* avoid division by zero */
    ...
    lib80211_extract_legacy_symbol(
        plan, data_sym_re, data_sym_im, H_re, H_im, noise_var, 0, &pilot_state, eq_re, eq_im);

    /* FFT the data symbol to get freq-domain input for equalize bench */
    float fft_out_re[64], fft_out_im[64];
    lib80211_fft_forward(plan, data_sym_re + 16, data_sym_im + 16, fft_out_re, fft_out_im);
    ...
    {
        int iters =
            lib80211_ldpc_decode(ldpc_llr, ldpc_decoded, ldpc_cw_len, ldpc_rate_n, ldpc_rate_d, 30);
