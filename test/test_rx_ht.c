...
static void test_loopback(lib80211_fft_plan *plan, int mcs, bool short_gi) {
    char test_name[64];
    snprintf(test_name, sizeof(test_name), "rx_ht_mcs%d%s", mcs, short_gi ? "_sgi" : "");
    ...
    size_t offset = 100;
    size_t n_tx   = lib80211_tx_ht(plan, &tx_params, tx_real + offset, tx_imag + offset);
    if (n_tx == 0) {
...
static void test_decode_vector(lib80211_fft_plan *plan, int mcs, bool short_gi) {
    char test_name[64];
    snprintf(test_name, sizeof(test_name), "rx_ht_vec_mcs%d%s", mcs, short_gi ? "_sgi" : "");
    TEST_BEGIN(test_name);

    char vec_name[64];
    snprintf(vec_name, sizeof(vec_name), "ht_mcs%d%s_waveform", mcs, short_gi ? "_sgi" : "");
...
static void test_loopback_ldpc(lib80211_fft_plan *plan, int mcs, bool short_gi) {
    char test_name[64];
    snprintf(test_name, sizeof(test_name), "rx_ht_ldpc_mcs%d%s", mcs, short_gi ? "_sgi" : "");
    TEST_BEGIN(test_name);
