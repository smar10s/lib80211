/**
 * test_rx_vht.c -- Validate VHT (802.11ac) frame decode pipeline.
 *
 * Tests:
 * 1. TX->RX loopback at all 9 MCS (BCC, normal GI)
 * 2. TX->RX loopback with short GI
 * 3. Decode TGac golden waveform vectors
 * 4. TX->RX loopback with LDPC coding
 */
...
static void test_loopback(lib80211_fft_plan *plan, int mcs, bool short_gi) {
    ...
    /* Check frame type */
    if (result.type != LIB80211_FRAME_VHT) {
        printf("    type: expected VHT(%d), got %d\n", LIB80211_FRAME_VHT, result.type);
        ok = 0;
    }
...
static void test_decode_vector(lib80211_fft_plan *plan, int mcs, bool short_gi) {
    ...
    if (result.type != LIB80211_FRAME_VHT) {
        printf("    type: expected VHT, got %d\n", result.type);
        ok = 0;
    }
...
int main(void) {
    ...
    /* TX->RX loopback: all 9 MCS, normal GI */
    ...
    test_loopback(plan, 8, true);

    /* TGac golden waveform vectors — normal GI */
    printf("\ntest_rx_vht: TGac golden vectors (MCS 0-8)\n");
    for (int mcs = 0; mcs <= 8; mcs++) {
        test_decode_vector(plan, mcs, false);
    }

    /* TGac golden waveform vectors — short GI */
    for (int mcs = 0; mcs <= 8; mcs++) {
        test_decode_vector(plan, mcs, true);
    }

    /* TX->RX loopback: LDPC coded, all 9 MCS */
    printf("\ntest_rx_vht: VHT frame decode (MCS 0-8, LDPC)\n");
    for (int mcs = 0; mcs <= 8; mcs++) {
    ...
    test_loopback_ldpc(plan, 8, true);
