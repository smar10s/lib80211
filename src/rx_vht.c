...
static int decode_vhtsiga(
    const lib80211_rx_state *state, int *mcs, bool *short_gi, bool *ldpc, int *ldpc_extra) {
...
static int decode_vhtsigb(lib80211_fft_plan *plan,
    ...
    lib80211_fft_forward(
        plan, sigb_re + LIB80211_NCP, sigb_im + LIB80211_NCP, freq_real, freq_imag);

    float eq_real[64], eq_imag[64];
    lib80211_equalize(freq_real, freq_imag, H_real, H_imag, noise_var, eq_real, eq_imag);
...
static void refine_channel_from_sigb(lib80211_fft_plan *plan,
    ...
    lib80211_fft_forward(plan, sigb_re + LIB80211_NCP, sigb_im + LIB80211_NCP, Y_real, Y_imag);
...
int lib80211_rx_decode_vht(const lib80211_rx_state *state,
    ...
    if (ldpc) {
        ...
        lib80211_pilot_state data_pilot = {
            .slope = 0.0f, .alpha = 0.3f, .initialized = 0, .update_h = (n_symbols > 3)};
        ...
        for (int i = 0; i < n_symbols; i++) {
            ...
            lib80211_extract_ht_vht_symbol(state->plan,
                                           ...
                                           i,
                                           ncp,
                                           LIB80211_VHT_PILOT_Z_START,
                                           &data_pilot,
                                           sym_data_re,
                                           sym_data_im);

            lib80211_soft_demap(sym_data_re, sym_data_im, sym_soft, LIB80211_N_HT_DATA_SC, n_bpsc);

            memcpy(all_soft + (size_t)i * (size_t)n_cbps, sym_soft, (size_t)n_cbps * sizeof(float));
        }
        ...
        int rc = lib80211_ldpc_decode_data(all_soft,
                                           total_soft,
                                           decoded_bits,
                                           n_symbols,
                                           n_dbps,
    ...
    /* BCC path */
    {
        ...
        for (int i = 0; i < n_symbols; i++) {
            ...
            memcpy(
                all_soft + (size_t)i * (size_t)n_cbps, sym_deint, (size_t)n_cbps * sizeof(float));
        }
        ...
        if (scramble_len > n_data_bits)
            scramble_len = n_data_bits;
        return lib80211_rx_bcc_decode(all_soft,
                                      total_soft,
                                      n_symbols,
                                      n_dbps,
                                      mcs_info->cr_n,
