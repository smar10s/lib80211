...
int lib80211_ldpc_decode(const float *llr_in,
                         ...
                         int max_iterations) {
    if (max_iterations <= 0)
        max_iterations = LDPC_MAX_ITERS_DEFAULT;
    ...
    for (int i = 0; i < M; i++) {
        for (int k = 0; k < degree[i]; k++) {
            ...
            for (int r = 0; r < Z; r++) {
                int c = r + shift;
                if (c >= Z)
                    c -= Z; /* branchless-friendly modulo for 0 <= r,shift < Z */
...
static const ldpc_h_desc *get_h_desc(int cw_len, int rate_n, int rate_d) {
    ...
    int rate_idx;
    if (rate_n == 1 && rate_d == 2)
        rate_idx = 0;
    else if (rate_n == 2 && rate_d == 3)
        rate_idx = 1;
    else if (rate_n == 3 && rate_d == 4)
        rate_idx = 2;
    else if (rate_n == 5 && rate_d == 6)
        rate_idx = 3;
    else
        return NULL;
...
int lib80211_ldpc_decode_data(const float *llr_in,
    ...
    for (int j = 0; j < n_cw; j++) {
        ...
        int copy_len = k_j;
        if (out_offset + copy_len > n_pld)
            copy_len = n_pld - out_offset;
        if (copy_len > 0) {
