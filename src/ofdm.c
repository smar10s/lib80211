...
void lib80211_generate_stf(lib80211_fft_plan *plan, float *out_real, float *out_imag) {
    ...
    float time_real[64], time_imag[64];
    lib80211_fft_inverse(
        plan, LIB80211_STF_FREQ_REAL, LIB80211_STF_FREQ_IMAG, time_real, time_imag);
...
void lib80211_generate_ltf(lib80211_fft_plan *plan, float *out_real, float *out_imag) {
    ...
    float time_real[64], time_imag[64];
    lib80211_fft_inverse(plan, LIB80211_LTF_FREQ_REAL, zero_imag, time_real, time_imag);
...
void lib80211_make_ht_ofdm_symbol(lib80211_fft_plan *plan,
    ...
    float polarity =
        (float)LIB80211_PILOT_POLARITY[(LIB80211_HT_PILOT_Z_START + symbol_index) % 127];
...
void lib80211_generate_ht_ltf(lib80211_fft_plan *plan, float *out_real, float *out_imag) {
    ...
    float zero_imag[64] = {0};
    float time_real[64], time_imag[64];
    lib80211_fft_inverse(plan, LIB80211_HT_LTF_FREQ_REAL, zero_imag, time_real, time_imag);
