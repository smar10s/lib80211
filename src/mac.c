...
static size_t write_mgmt_header(
    uint8_t *buf, uint16_t fc, const uint8_t *da, const uint8_t *sa, const uint8_t *bssid) {
...
size_t lib80211_build_beacon(uint8_t *buf,
     ...
     */
    size_t total =
        24 + 8 + 2 + 2 + (2 + ssid_len) + (2 + sizeof(SUPPORTED_RATES)) + (2 + 1) + (2 + 4);
    ...
    pos += write_mgmt_header(buf + pos, LIB80211_FC_BEACON, LIB80211_BROADCAST, bss, bss);

    /* Fixed fields */
    put_le64(buf + pos, timestamp);
    pos += 8;
    put_le16(buf + pos, beacon_interval);
    pos += 2;
    put_le16(buf + pos, LIB80211_CAPABILITIES_DEFAULT);
    pos += 2;
...
size_t lib80211_build_assoc_req(
    ...
    pos += write_mgmt_header(buf + pos, LIB80211_FC_ASSOC_REQ, bssid, sta_mac, bssid);

    put_le16(buf + pos, LIB80211_CAPABILITIES_DEFAULT);
    pos += 2;
    put_le16(buf + pos, 100);
    pos += 2;
...
lib80211_build_auth(uint8_t *buf, size_t buf_len, const uint8_t *bssid, const uint8_t *sta_mac) {
    if (!buf || !bssid || !sta_mac)
        return 0;
    if (buf_len < 30)
        return 0;

    size_t pos = 0;
    pos += write_mgmt_header(buf + pos, LIB80211_FC_AUTH, bssid, sta_mac, bssid);
    put_le16(buf + pos, 0);
    pos += 2; /* Algorithm: Open System */
    put_le16(buf + pos, 1);
    pos += 2; /* Seq: 1 */
    put_le16(buf + pos, 0);
    pos += 2; /* Status: 0 (reserved in request) */
