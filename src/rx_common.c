...
int lib80211_rx_front_end_s(lib80211_fft_plan *plan,
    ...
    if (lib80211_sync_detect_with_work(
            plan, iq_real, iq_imag, n_samples, state->work_re, state->work_im, &sync) != 0)
        return -1;
    ...
    if (state->noise_var < 1e-6f)
        state->noise_var = 1e-6f;
...
int lib80211_rx_bcc_decode(float *all_soft,
    ...
    if (depunct_len < n_coded_needed) {
        ...
        float *viterbi_input =
            (float *)lib80211_scratch_alloc(scratch, n_coded_needed * sizeof(float), 0);
        if (!viterbi_input)
            return -1;
        memcpy(viterbi_input, depunctured, depunct_len * sizeof(float));
        memset(viterbi_input + depunct_len, 0, (n_coded_needed - depunct_len) * sizeof(float));
