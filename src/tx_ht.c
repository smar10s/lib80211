...
size_t lib80211_tx_ht_samples(const lib80211_tx_ht_params *params) {
    if (params->mcs < 0 || params->mcs > 7)
        return 0;
    if (params->psdu_len > HT_MAX_PSDU)
        return 0;
...
size_t lib80211_tx_ht_s(lib80211_fft_plan *plan,
    ...
    lib80211_make_htsig_symbols(plan,
                                params->mcs,
                                (int)params->psdu_len,
                                params->short_gi,
                                params->ldpc,
    ...
    if (params->ldpc) {
        ...
        /* Scramble */
        lib80211_scramble(data_bits, scrambled, (size_t)scramble_len, params->scrambler_seed);

        /* LDPC encode (handles codeword selection, shortening, puncturing, extra symbol) */
        int n_sym_ldpc = 0, ldpc_extra = 0;
        lib80211_ldpc_encode_data(
            scrambled, n_payload, n_dbps, n_cbps, cr_n, cr_d, coded, &n_sym_ldpc, &ldpc_extra);
        ...
        for (int s = 0; s < n_sym_ldpc; s++) {
            ...
            /* HT OFDM symbol */
            lib80211_make_ht_ofdm_symbol(
                plan, mod_real, mod_imag, s, cp_len, &out_real[out_idx], &out_imag[out_idx]);
